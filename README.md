...
PHPだけで動く、軽量お知らせ管理CMSテンプレートです。
データベース不要（JSONでデータ管理）なので、格安レンタルサーバーでもすぐ使えます。

「お知らせページだけ自分で更新したい」「既存のCMSサイトでは重すぎる」という
小規模サイト向けに作りました。

## 特徴
- DB不要、JSON管理だからバックアップも楽
- 管理画面からお知らせを追加・編集するだけ
- レンタルサーバーにそのままアップロードでOK
...
└── cms/                   # CMSシステム一式
    ...
    │   ├── news.json      # お知らせデータ
    │   ├── users.json     # 管理者アカウント
    │   └── users.json.example  # 初期ユーザーファイル（リネームして使用）
    ├── uploads/           # アップロード画像
